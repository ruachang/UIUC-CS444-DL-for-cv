...
class NeuralNetwork:
    ...
    def __init__(
        ...
        for i in range(1, num_layers + 1):
            self.params["W" + str(i)] = np.random.randn(self.sizes[i - 1], self.sizes[i]) / np.sqrt(self.sizes[i - 1])
    ...
    def SGD_optimizer(self, lr: float):
        self.lr = lr
        return
    def Adam_optimizer(self, lr: float, **kwargs):
        self.b1 = kwargs["b1"]
        self.b2 = kwargs["b2"]
        self.eps = kwargs["eps"]
        self.lr = lr
        ...
        for i in range(1, self.num_layers + 1):
            self.m["W" + str(i)] = np.zeros((self.sizes[i - 1], self.sizes[i]))
            self.m["b" + str(i)] = np.zeros(self.sizes[i])
            self.v["W" + str(i)] = np.zeros((self.sizes[i - 1], self.sizes[i]))
            self.v["b" + str(i)] = np.zeros(self.sizes[i])
        return
    ...
    def update(
        ...
        if opt == "SGD":
            for i in range(1, self.num_layers + 1):
                ...
                self.params["W" + str(i)] += -self.lr * self.gradients["W" + str(i)]
                self.params["b" + str(i)] += -self.lr * self.gradients["b" + str(i)]
                # print(self.params["W" + str(i)], params["b" + str(i)])
        else:
            for i in range(1, self.num_layers + 1):
                self.m["W" + str(i)] = self.b1 * self.m["W" + str(i)] + (1 - self.b1) * self.gradients["W" + str(i)]
                self.v["W" + str(i)] = self.b2 * self.v["W" + str(i)] + (1 - self.b2) * (self.gradients["W" + str(i)] ** 2)
                # print("W" + str(i), self.lr, self.eps)
                self.params["W" + str(i)] += -self.lr * self.m["W" + str(i)] / (np.sqrt(self.v["W" + str(i)]) + self.eps)
                
                self.m["b" + str(i)] = self.b1 * self.m["b" + str(i)] + (1 - self.b1) * self.gradients["b" + str(i)]
                self.v["b" + str(i)] = self.b2 * self.v["b" + str(i)] + (1 - self.b2) * (self.gradients["b" + str(i)] ** 2)
                self.params["b" + str(i)] += -self.lr * self.m["b" + str(i)] / (np.sqrt(self.v["b" + str(i)]) + self.eps)
        return
...
def pridict_dataloader(model, dataloader, batch_num=None):
    """
    Predict output for a dataloader
    ...
    with torch.no_grad():
        ...
        for pred_num in range(all_pred.shape[0]):
            pred = all_pred[pred_num].cpu()
            boxes, cls_indexs, probs = decoder(pred)
            for i, box in enumerate(boxes):
                x1 = int(box[0] * w)
                x2 = int(box[2] * w)
                ...
                prob = float(prob)
                result.append(
                    [[(x1, y1), (x2, y2), VOC_CLASSES[cls_index], pred_num, prob]]
                )
    return result
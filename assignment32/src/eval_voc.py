...
def evaluate(model, test_dataset_file, img_root, test_loader=None, batch_num=None):
    ...
    f.close()
    if batch_num is not None and test_loader is not None:
        batch_size = test_loader.batch_size
        file_list = file_list[: batch_num * batch_size]
    ...
    if test_loader is not None:
        results = pridict_dataloader(model, test_loader, batch_num)
        print("---Get all sample results---")
        for result in tqdm(results):
            for (
                ...
                preds[class_name].append([image_list[image_id], prob, x1, y1, x2, y2])
    else:
